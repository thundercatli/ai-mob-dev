...
- **连接管理**：多个 SSH 连接配置，支持命名、复制、删除；凭据存在 EncryptedSharedPreferences 里
...
`.github/workflows/android.yml` 在 push / PR 时自动构建 frpc 和 debug APK，产物作为 artifact `app-debug` 上传。也可以在 Actions 页面手动触发（`workflow_dispatch`），可指定 frp 版本号。
